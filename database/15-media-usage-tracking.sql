-- Media Usage Tracking System Enhancement
...
-- RLS Policies for media usage tracking
CREATE POLICY "Users can view usage for their content" ON media_usage_tracking
    FOR SELECT USING (
        -- Users can see usage for media they uploaded
        EXISTS (
...
-- Function to find orphaned media (not used anywhere)
...
BEGIN
    ...
    WHERE (user_id IS NULL OR ma.uploaded_by = user_id)
    AND NOT EXISTS (
...
-- Backfill existing gallery media associations
INSERT INTO media_usage_tracking (
    media_asset_id,
    used_in_type,
    used_in_id,
