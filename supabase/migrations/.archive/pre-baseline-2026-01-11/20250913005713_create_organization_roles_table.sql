...
BEGIN
  ...
  -- Check role hierarchy: elder > cultural_keeper > knowledge_holder > admin > project_leader > storyteller > community_member > guest
  RETURN CASE
    WHEN required_role = 'guest' THEN user_role IS NOT NULL
    WHEN required_role = 'community_member' THEN user_role IN ('community_member', 'storyteller', 'project_leader', 'admin', 'knowledge_holder', 'cultural_keeper', 'elder', 'cultural_liaison', 'archivist')
    WHEN required_role = 'storyteller' THEN user_role IN ('storyteller', 'project_leader', 'admin', 'knowledge_holder', 'cultural_keeper', 'elder', 'archivist')
    WHEN required_role = 'project_leader' THEN user_role IN ('project_leader', 'admin', 'knowledge_holder', 'cultural_keeper', 'elder')
    WHEN required_role = 'admin' THEN user_role IN ('admin', 'knowledge_holder', 'cultural_keeper', 'elder')
    ...
    WHEN required_role = 'cultural_liaison' THEN user_role IN ('cultural_liaison', 'admin', 'cultural_keeper', 'elder')
    WHEN required_role = 'archivist' THEN user_role IN ('archivist', 'admin', 'cultural_keeper', 'elder')
    ELSE false
  END;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
