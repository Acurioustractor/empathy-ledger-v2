...
-- Get organization_id: SELECT id FROM organizations LIMIT 1;
...
-- Goods Asset Register (Asset Management & Community Resources)
INSERT INTO syndication_sites (
  slug,
  name,
  description,
  ...
  'Goods Asset Register',
  'Community asset management and resource sharing platform',
  'Your story helps communities understand the value of shared resources and collective ownership',
  'Community organizers, asset managers, and resource coordinators',
  'contact@goodsassetregister.org',
  'Goods Asset Register Team',
  'https://goodsassetregister.org/api/webhooks/empathy-ledger',
  'https://goodsassetregister.org/api',
  'dev-goods-asset-register-key-hash',
  ARRAY['goodsassetregister.org', 'www.goodsassetregister.org'],
  'active',
  (SELECT id FROM organizations LIMIT 1)
) ON CONFLICT (slug) DO NOTHING;

-- ACT Farm (Regenerative Agriculture & Land Stewardship)
INSERT INTO syndication_sites (
  slug,
  name,
  description,
  ...
  'act-farm',
  'ACT Farm',
  'Regenerative agriculture and land stewardship stories from Country',
  'Your story connects people to Country and demonstrates regenerative practices that heal the land',
  'Farmers, land stewards, Indigenous rangers, and environmental practitioners',
  'contact@actfarm.org.au',
  'ACT Farm Team',
  'https://actfarm.org.au/api/webhooks/empathy-ledger',
  'https://actfarm.org.au/api',
  'dev-act-farm-key-hash',
  ARRAY['actfarm.org.au', 'www.actfarm.org.au'],
  'active',
  (SELECT id FROM organizations LIMIT 1)
) ON CONFLICT (slug) DO NOTHING;

-- Regenerative Innovation Studio (Innovation & Systems Change)
INSERT INTO syndication_sites (
  slug,
  name,
  description,
  ...
  'regenerative-innovation-studio',
  'Regenerative Innovation Studio',
  'Stories of innovation, systems change, and regenerative design from the edge of what''s possible',
  'Your story inspires others to reimagine systems and design for regeneration at all scales',
  'Innovators, designers, systems thinkers, and changemakers',
  'contact@regenerativeinnovation.studio',
  'Regenerative Innovation Studio Team',
  'https://regenerativeinnovation.studio/api/webhooks/empathy-ledger',
  'https://regenerativeinnovation.studio/api',
  'dev-regenerative-innovation-studio-key-hash',
  ARRAY['regenerativeinnovation.studio', 'www.regenerativeinnovation.studio'],
  'active',
  (SELECT id FROM organizations LIMIT 1)
) ON CONFLICT (slug) DO NOTHING;
