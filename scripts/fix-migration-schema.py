#!/usr/bin/env python3
"""
Fix syndication migration schema to match Empathy Ledger database structure.

Changes:
1. Replace 'tenant_id' with 'organization_id'
2. Replace 'tenants' table reference with 'organizations'
3. Replace 'tenant_members' table reference with 'organization_members'
4. Remove 'auth.users' references (no auth schema)
5. Remove 'auth' role references (authenticated, service_role)
"""
...
def fix_migration(input_file, output_file):
    ...
    # Pattern: "UUID REFERENCES auth.users(id)" -> "UUID -- No auth schema"
    content = re.sub(
        r'UUID REFERENCES auth\.users\(id\)([^,]*)?',
        r'UUID -- No auth schema',
        content
    )

    # Fix 5: Remove authenticated/service_role grant statements
    # Pattern: "GRANT ... TO authenticated;" -> "-- GRANT ... TO authenticated; (no auth roles)"
    content = re.sub(
