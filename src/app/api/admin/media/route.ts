    ...
    if (mediaAssets && mediaAssets.length > 0) {
      processedAssets = mediaAssets.map(asset => {
        ...
        return {
          ...
          publicUrl: asset.cdn_url || (asset.storage_path ? `${process.env.NEXT_PUBLIC_SUPABASE_URL || 'https://yvnuayzslukamizrlhwb.supabase.co'}/storage/v1/object/public/${asset.storage_bucket || 'media'}/${asset.storage_path}` : ''),
