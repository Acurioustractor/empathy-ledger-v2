...
export default function OrganizationsAdminPage() {
  ...
  useEffect(() => {
    filterOrganizations()
  }, [organizations, searchTerm, statusFilter, typeFilter, verificationFilter, activityFilter])
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600">
              Manage organizations and their tenant settings
            </p>
          </div>
          <Button
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-2xl font-bold">{organizations.filter(o => o.type === 'community').length}</p>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-2xl font-bold">{organizations.filter(o => o.type === 'nonprofit').length}</p>
                <p className="text-gray-600">Nonprofit</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-2xl font-bold">{organizations.filter(o => o.type === 'cultural_center').length}</p>
                <p className="text-gray-600">Cultural Centers</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        {filteredOrganizations.map((org) => (
          <Card key={org.id} className="overflow-hidden hover:shadow-lg transition-shadow">
            ...
            <CardContent className="space-y-4">
              ...
              {/* Cultural Protocols */}
              {org.cultural_protocols && Object.keys(org.cultural_protocols).length > 0 && (
                <div className="p-3 bg-blue-50 rounded-md">
                  <div className="flex items-center mb-2">
                    <Shield className="w-4 h-4 text-blue-600 mr-1" />
                    <p className="text-sm text-blue-800 font-medium">Cultural Protocols</p>
                  </div>
                  <div className="space-y-1">
                    {Object.entries(org.cultural_protocols).slice(0, 2).map(([key, value]) => (
                      <div key={key} className="text-xs text-blue-700">
                        <span className="font-medium">{key.replace('_', ' ')}:</span> {value}
                      </div>
                    ))}
                    {Object.keys(org.cultural_protocols).length > 2 && (
                      <p className="text-xs text-blue-600 italic">+{Object.keys(org.cultural_protocols).length - 2} more protocols</p>
                    )}
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-4 p-3 bg-gray-50 rounded-md">
                <div className="text-center">
                  <div className="text-lg font-bold text-gray-900">{org.story_count || 0}</div>
                  <div className="text-xs text-gray-600">Stories</div>
                </div>
                <div className="text-center">
                  <div className="text-lg font-bold text-gray-900">{org.member_count || 0}</div>
