...
import EntityTagPicker from '@/components/admin/EntityTagPicker'
...
export default function PhotosAdminPage() {
  ...
  const [isEntityTaggingOpen, setIsEntityTaggingOpen] = useState(false)
  const [photoToEntityTag, setPhotoToEntityTag] = useState<Photo | null>(null)
  ...
  const saveEntityTags = async (data: {
    storytellerIds: string[]
    projectId: string | null
    organizationId: string | null
  }) => {
    if (!photoToEntityTag) return
    ...
    try {
      const response = await fetch('/api/admin/media', {
        ...
        body: JSON.stringify({
          id: photoToEntityTag.id,
          taggedStorytellerIds: data.storytellerIds,
      ...
      if (response.ok) {
        // Update local state - will need to refetch to get full entity names
        fetchPhotos()
        setIsEntityTaggingOpen(false)
        setPhotoToEntityTag(null)
      } else {
        console.error('Failed to update entity tags')
        alert('Failed to update entity tags. Please try again.')
      }
    } catch (error) {
      console.error('Error updating entity tags:', error)
      alert('Error updating entity tags. Please try again.')
    }
  }

  const openEntityTagging = (photo: Photo) => {
    setPhotoToEntityTag(photo)
    setIsEntityTaggingOpen(true)
  }
...
This will help organise your media collection.`
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'active': return 'bg-sage-100 text-sage-700 border border-sage-200'
      case 'flagged': return 'bg-clay-100 text-clay-700 border border-clay-200'
      case 'hidden': return 'bg-stone-100 text-stone-600 border border-stone-200'
  ...
  const getVisibilityColor = (visibility: string) => {
    switch (visibility) {
      case 'public': return 'bg-sage-100 text-sage-700 border border-sage-200'
      case 'community': return 'bg-earth-100 text-earth-700 border border-earth-200'
      case 'private': return 'bg-stone-100 text-stone-600 border border-stone-200'
      default: return 'bg-stone-100 text-stone-600 border border-stone-200'
    }
  }
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-center h-64">
          ...
          <span className="ml-2 text-stone-600">Loading photos...</span>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="bg-gradient-to-r from-stone-50 via-sage-50/30 to-earth-50/20 border border-stone-200 rounded-xl px-6 py-6">
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-display-sm font-bold tracking-tight text-stone-900">Photos Admin</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
        <Card className="bg-gradient-to-br from-stone-50 to-stone-100/50 border-stone-200">
          <CardContent className="p-4">
            <div className="text-center">
              <p className="text-2xl font-bold text-stone-800">{totalPhotos}</p>
        ...
        <Card className="bg-gradient-to-br from-sage-50 to-sage-100/50 border-sage-200">
          <CardContent className="p-4">
            <div className="text-center">
              <p className="text-2xl font-bold text-sage-700">{activePhotos}</p>
        ...
        <Card className="bg-gradient-to-br from-clay-50 to-clay-100/50 border-clay-200">
          <CardContent className="p-4">
            <div className="text-center">
              <p className="text-2xl font-bold text-clay-700">{flaggedPhotos}</p>
        ...
        <Card className="bg-gradient-to-br from-earth-50 to-earth-100/50 border-earth-200">
          <CardContent className="p-4">
            <div className="text-center">
              <p className="text-2xl font-bold text-earth-700">{linkedPhotos}</p>
        ...
        <Card className="bg-gradient-to-br from-amber-50 to-amber-100/50 border-amber-200">
          <CardContent className="p-4">
            <div className="text-center">
              <p className="text-2xl font-bold text-amber-700">{unlinkedPhotos}</p>
        ...
        <Card className="bg-gradient-to-br from-clay-50 to-clay-100/50 border-clay-200">
          <CardContent className="p-4">
            <div className="text-center">
              <p className="text-2xl font-bold text-clay-700">{highSensitivityPhotos}</p>
        ...
        <Card className="bg-gradient-to-br from-sage-50 to-sage-100/50 border-sage-200">
          <CardContent className="p-4">
            <div className="text-center">
              <p className="text-2xl font-bold text-sage-700">{Math.round(totalSize)}</p>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Button
                ...
                className={viewMode === 'grid' ? 'bg-gradient-to-r from-sage-600 to-sage-700 hover:from-sage-700 hover:to-sage-800' : 'border-stone-300'}
              >
              ...
              <Button
                ...
                className={viewMode === 'list' ? 'bg-gradient-to-r from-sage-600 to-sage-700 hover:from-sage-700 hover:to-sage-800' : 'border-stone-300'}
              >
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                className={isBatchMode ? 'bg-gradient-to-r from-sage-600 to-sage-700 hover:from-sage-700 hover:to-sage-800' : 'border-stone-300'}
              >
                  ...
                  {selectedPhotos.size > 0 && (
                    <>
                      <Button
                        ...
                        className="text-sage-700 border-sage-300 hover:bg-sage-50"
                      >
              ...
              <div className="border-l border-stone-200 pl-2 ml-2">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  className="ml-2 border-stone-300"
                ...
                <Button variant="outline" size="sm" className="ml-2 border-stone-300">
          ...
          {filteredPhotos.map((photo, index) => (
            <Card key={photo.id} className="overflow-hidden hover:shadow-lg transition-all duration-200 cursor-pointer group">
              <div onClick={() => handleViewPhoto(photo)} className="relative">
                <div className="aspect-[4/3] bg-gradient-to-br from-stone-100 to-earth-100 flex items-center justify-center relative overflow-hidden">
                  {photo.publicUrl ? (
                    <img
                      src={photo.publicUrl}
                      alt={photo.title}
                ...
                <div className="absolute top-3 left-3 flex gap-2">
                  ...
                  {photo.galleries.length > 0 && (
                    <Badge className="bg-earth-600 text-white text-xs font-medium shadow-sm">
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black via-black/80 to-transparent p-4">
                  ...
                  <div className="flex items-center justify-between">
                    <p className="text-earth-200 text-xs truncate">{photo.organizationName}</p>
                    <p className="text-earth-300 text-xs font-medium">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    {photo.consentStatus === 'granted' && (
                      <Badge className="bg-sage-100 text-sage-800" size="sm">
              ...
              <CardContent className="p-4">
                <div className="space-y-2">
                  ...
                  {/* Entity Tags (People, Projects, Organizations) */}
                  {(photo.taggedStorytellers?.length > 0 || photo.projectName || photo.organizationName) && (
                    <div className="flex flex-wrap gap-1">
                      {photo.taggedStorytellers?.slice(0, 2).map(person => (
                        <Badge key={person.id} variant="outline" className="text-xs bg-sage-50 border-sage-200 text-sage-700">
                          <User className="w-3 h-3 mr-1" />
                          {person.name}
                        </Badge>
                      ))}
                      {photo.taggedStorytellers?.length > 2 && (
                        <Badge variant="outline" className="text-xs bg-sage-50 border-sage-200 text-sage-700">
                          +{photo.taggedStorytellers.length - 2} more
                        </Badge>
                      )}
                      {photo.projectName && (
                        <Badge variant="outline" className="text-xs bg-earth-50 border-earth-200 text-earth-700">
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-stone-200">
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      className="text-clay-600 hover:text-clay-700 hover:bg-clay-50"
                      onClick={(e) => {
                        e.stopPropagation();
                    ...
                    <tr key={photo.id} className="hover:bg-stone-50">
                      ...
                      <td className="px-4 py-4">
                        <div className="flex items-center">
                          <div className="w-16 h-16 bg-earth-100 rounded-lg overflow-hidden mr-4 flex-shrink-0">
                            {photo.publicUrl ? (
                              <img
                                src={photo.publicUrl}
                                alt={photo.title}
          ...
          {selectedPhoto && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                <div className="aspect-square bg-gradient-to-br from-stone-100 to-earth-100 rounded-lg overflow-hidden relative">
                  ...
                  {selectedPhoto.publicUrl && (
                    <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black via-black/50 to-transparent p-4">
                      ...
                      <p className="text-earth-200 text-sm">{selectedPhoto.width} × {selectedPhoto.height}</p>
                    </div>
                  )}
                </div>
                ...
                <div className="flex justify-between items-center">
                  <Button
                    ...
                    size="sm"
                    onClick={() => {
                      if (selectedPhoto.publicUrl) {
                        window.open(selectedPhoto.publicUrl, '_blank')
                      }
                    }}
                    disabled={!selectedPhoto.publicUrl}
                  >
                    <Download className="w-4 h-4 mr-2" />
                    Download
                  </Button>
                  <div className="flex gap-2">
                    <Button
                      ...
                      onClick={() => {
                        ...
                        openEntityTagging(selectedPhoto)
                      }}
                    >
                ...
                <Card>
                  ...
                  <CardContent className="space-y-3">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <Badge className={
                        selectedPhoto.consentStatus === 'granted' ? 'bg-sage-100 text-sage-800 border border-sage-200' :
                        selectedPhoto.consentStatus === 'pending' ? 'bg-amber-100 text-amber-800 border border-amber-200' :
      ...
      <Dialog open={isDeleteConfirmOpen} onOpenChange={setIsDeleteConfirmOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-clay-600">
      ...
      {/* Entity Tagging Dialog */}
      <EntityTagPicker
        open={isEntityTaggingOpen}
        onOpenChange={setIsEntityTaggingOpen}
        selectedStorytellerIds={photoToEntityTag?.taggedStorytellerIds || []}
        selectedProjectId={photoToEntityTag?.projectId || null}
        selectedOrganizationId={photoToEntityTag?.organizationId || null}
        onSave={saveEntityTags}
        title="Tag People & Projects"
        photoPreview={photoToEntityTag ? {
          url: photoToEntityTag.publicUrl,
          title: photoToEntityTag.title
        } : undefined}
      />
    </div>
  )
