...
import TagAutocomplete, { type SelectedTag } from '@/components/media/TagAutocomplete'
import { ThemeAutocomplete } from '@/components/stories/ThemeAutocomplete'
import EnhancedMediaPicker from '@/components/admin/EnhancedMediaPicker'
import { SEOPreview } from '@/components/stories/SEOPreview'
...
export default function UnifiedContentFields({
  ...
  const [featuredImageTitle, setFeaturedImageTitle] = useState<string | null>(null)

  // Load featured image details if ID exists
  useEffect(() => {
    async function loadFeaturedImage() {
      if (formData.featured_image_id && !featuredImageUrl) {
        try {
          const response = await fetch(`/api/media?id=${formData.featured_image_id}`)
          if (response.ok) {
            const data = await response.json()
            if (data.media && data.media.length > 0) {
              const media = data.media[0]
              setFeaturedImageUrl(media.cdn_url || media.storage_url)
              setFeaturedImageTitle(media.title || 'Featured Image')
            }
          }
        } catch (error) {
  ...
  // Convert tag objects to/from string arrays
  const handleTagsChange = (tags: SelectedTag[]) => {
    const tagNames = tags.map(tag => tag.name)
    onChange('tags', tagNames)
  }

  // Convert theme strings to/from string arrays
  const handleThemesChange = (themes: string[]) => {
    onChange('themes', themes)
  }

  // Convert formData.tags to SelectedTag[] format
  const selectedTags: SelectedTag[] = (formData.tags || []).map((tagName: string) => ({
    ...
    <div className="space-y-6">
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <TagAutocomplete
              ...
              placeholder="Add tags (e.g., trauma, healing, justice)"
              allowCreate={true}
              showCategoryFilter={true}
              maxTags={20}
            />
            <p className="text-xs text-muted-foreground">
              Add descriptive tags to help categorize and discover your content
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <ThemeAutocomplete
              ...
              placeholder="Select themes (e.g., Resilience, Community, Cultural Identity)"
              maxThemes={10}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Primary image for story cards and social sharing
          </CardDescription>
        </CardHeader>
        <CardContent>
          {formData.featured_image_id && featuredImageUrl ? (
            <div className="flex items-center gap-4 p-4 border rounded-lg">
              {/* Image preview */}
              <div className="relative w-32 h-32 rounded overflow-hidden bg-stone-100 dark:bg-stone-800">
                <Image
                  src={featuredImageUrl}
                  alt={featuredImageTitle || 'Featured'}
                  fill
                  className="object-cover"
                />
              </div>
              <div className="flex-1">
                <p className="font-medium">{featuredImageTitle}</p>
                <div className="flex gap-2 mt-2">
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={() => {
                      onChange('featured_image_id', null)
                      setFeaturedImageUrl(null)
                      setFeaturedImageTitle(null)
                    }}
                    type="button"
                  >
      ...
      {/* Enhanced Media Picker Dialog */}
      <EnhancedMediaPicker
        open={imagePickerOpen}
        onOpenChange={setImagePickerOpen}
        onSelect={(media) => {
          onChange('featured_image_id', media.id)
          setFeaturedImageUrl(media.cdn_url || '')
          setFeaturedImageTitle(media.title || 'Featured Image')
