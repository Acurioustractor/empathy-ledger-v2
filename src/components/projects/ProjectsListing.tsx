...
export function ProjectsListing({ projects }: ProjectsListingProps) {
  ...
  const filteredProjects = projects.filter(project => {
    ...
    const organisation = project.organisation?.name || ''
    
    const matchesSearch = name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      description.toLowerCase().includes(searchTerm.toLowerCase()) ||
      organisation.toLowerCase().includes(searchTerm.toLowerCase())
              ...
              <div className="pt-2 flex gap-2">
                ...
                {project.organisation && (
                  <Button asChild size="sm" variant="secondary">
                    <Link href={`/organisations/${project.organisation.id}/projects/${project.id}/manage`}>
